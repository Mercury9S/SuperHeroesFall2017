 ...
 */
  ...
  if (isset($_POST['villain_name_for_PHP'], $_POST['associated_superhero_for_PHP']))
  {
    try {
        $db = new PDO(DB_DRIVER . ":dbname=" . DB_DATABASE . ";host=" . DB_SERVER, DB_USER, DB_PASSWORD);
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $stmt = $db->prepare("INSERT INTO enemies (ArchNemesis, PublicName)
                                                 VALUES (:villain_name, :associated_superhero)");

        $stmt->bindParam(':villain_name', $_POST['villain_name_for_PHP'], PDO::PARAM_STR, 40);
